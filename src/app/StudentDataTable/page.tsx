...
const StudentDataTable = () => {
    ...
    const [experienceFilter, setExperienceFilter] = useState({ min: 0, max: 20 });
    const [contactFilter, setContactFilter] = useState('');
    const [cityFilter, setCityFilter] = useState('');

    // Handle delete function
    const handleDelete = async (contact) => {
        const confirmed = window.confirm(`Are you sure you want to delete, This response will no longer exit in table `);
    ...
    let filteredCandidates = data ? data.candidatesInfo : [];
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {filteredCandidates.length > 0 ? (
                            filteredCandidates.map(candidate => (
                                <tr key={candidate.id}>
                                     <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <button onClick={() => handleDelete(candidate.contact)} className="text-red-600 hover:text-red-900">
                                            Delete
                                        </button>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{candidate.student}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{candidate.working_professional}</td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{candidate.answer1}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{candidate.answer2}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{candidate.answer3}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{candidate.answer4}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{candidate.answer5}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><a href={candidate.resume} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">Click Here To View</a></td>
                            ...
                            <tr>
                                <td colSpan="19" className="px-6 py-4 text-center text-sm text-gray-500">No candidates found</td>
                            </tr>
                        )}
                    </tbody>
                </table>
